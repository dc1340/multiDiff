...
for (wt_coef in names(p2_wt_diff[[3]])){
  ...
  source('R/quick_multidiff_pipeline.R')
  #source('R/quick_heatmap.R')
  ##Cd sex combined, background filtered, self interesect
  if (wt_coef=='IsCd'){
    cur_coef='Is10ppm'
    cur_multi=filterMultiOnMulti(cur_multi, cur_multi, coef=cur_coef, meth.thresh = meth.thresh)
    
    
    job.id=paste('P2_L_Sex_Combined_Filtered_Background_SelfIntersect' , cur_coef, sep="_")
    source('R/quick_genic_annotation.R')
    source('R/quick_multidiff_pipeline.R')
  }
}
...
## Combined wiltypes to estimate background effects
...
cur_multi=filterMultiOnMulti(p2_sexcomb_l_diff, p2_wt_diff, coef='IsCd')
cur_coef='Is10ppm'
cur_multi=filterMultiOnMulti(cur_multi, cur_multi, coef=cur_coef, meth.thresh = meth.thresh)
...
##Cd sex combined, background filtered, self interesect
cur_multi=filterMultiOnMulti(p2_sexcomb_cd_diff, p2_wt_diff, coef='IsCd')
cur_coef='Is10ppm'
cur_multi=filterMultiOnMulti(cur_multi, cur_multi, coef=cur_coef, meth.thresh = meth.thresh)


job.id=paste('P2_Cd_Sex_Combined_Filtered_Background_SelfIntersect' , cur_coef, sep="_")
source('R/quick_genic_annotation.R')
source('R/quick_multidiff_pipeline.R')
#source('R/quick_heatmap.R')
